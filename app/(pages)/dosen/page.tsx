...
import { getLecturers, createLecturer, updateLecturer, deleteLecturer } from "@/app/actions/lecturers";
import Image from "next/image";

export default function LecturersPage() {
  const { successAction, confirmDeleteMessage, showError, showLoading } = useToastMessage();

  const [dataList, setDataList] = useState<Lecturer[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      showError(
        "Gagal Memuat Data",
        "Mohon maaf, sistem gagal memuat data dosen."
      );
    } finally {
      setIsLoading(false);
    }
